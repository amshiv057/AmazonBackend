...
(async () => {
    try {
        const result = await mongoose.model("products", productSchema).find({});
        // console.log("result Products",result);
        if (result.length > 0) {
            console.log("Default Products all ready Created!");
        } else {
            const obj1 = {
                id: 'product1',
            ...
            await mongoose.model("products", productSchema).create(obj1, obj2, obj3, obj4, obj5);
            console.log("Default products are created successfully!")
        }
    } catch (error) {
        console.log("Error occured in creating default products!",error);
    }
}).call();